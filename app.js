...
function filterData() {
  ...
  const filteredF2p = f2pData.filter((g) => {
    ...
    const platformMatch =
      platform === 'all' || platformText.includes(platform) || (platform === 'pc' && platformText.includes('windows'));
    return t.includes(q) && platformMatch;
  });
...
async function loadData() {
  ...
  try {
    const [giveaways, f2p] = await Promise.all([
      fetchJson(CORS + 'https://www.gamerpower.com/api/giveaways'),
      fetchJson(CORS + 'https://www.freetogame.com/api/games')
    ]);
